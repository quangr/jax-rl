...
class Critic(nn.Module):
    @nn.compact
    def __call__(self, x):
        x = nn.Dense(64, kernel_init=orthogonal(
            jnp.float_(np.sqrt(2))), bias_init=constant(0.0))(x)
        x = nn.tanh(x)
        x = nn.Dense(64, kernel_init=orthogonal(
            jnp.float_(np.sqrt(2))), bias_init=constant(0.0))(x)
        x = nn.tanh(x)
...
class Actor(nn.Module):
    ...
    def __call__(self, x):
        ...
        x = nn.Dense(64, kernel_init=orthogonal(
            jnp.float_(np.sqrt(2))), bias_init=constant(0.0))(x)
        x = nn.tanh(x)
        x = nn.Dense(self.action_dim, kernel_init=orthogonal(
            (0.01)*jnp.float_(np.sqrt(2))), bias_init=constant(0.0))(x)
        # stdlog = -jnp.ones((self.action_dim,))/2
        stdlog = self.param('stdlog', lambda rng, shape: -
...
if __name__ == "__main__":
    ...
    def step_env_wrappeed(episode_stats, handle, obs_rms, action):
        ...
        episode_stats = episode_stats.replace(
            ...
            returned_episode_returns=jnp.where(
                next_done+next_truncated, new_episode_return, episode_stats.returned_episode_returns
            ),
    ...
    def linear_schedule(count):
        ...
        frac = 1.0 - (count // (args.num_minibatches *
                      args.update_epochs)) / args.num_updates
    ...
    def get_action_and_value(
        ...
        logprob = (-((action-logits) ** 2) / (2 * var) - stdlog -
                   jnp.log(jnp.sqrt(2 * jnp.pi))).sum(-1)  # need gradient information
    ...
    def ppo_loss(params, x, a, logp, mb_advantages, mb_returns,truncated):
        ...
        pg_loss2 = -mb_advantages * \
            jnp.clip(ratio, 1 - args.clip_coef, 1 + args.clip_coef)
    ...
    for update in range(1, args.num_updates + 1):
        ...
        avg_episodic_length=np.mean(jax.device_get(
